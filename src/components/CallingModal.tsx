...
export default function CallingModal({
  ...
  // Attach remote stream to video or audio element based on call type
  useEffect(() => {
    if (remoteStream) {
      if (callState.type === 'video') {
        if (remoteVideoRef.current) {
          ...
          remoteVideoRef.current.play().catch((err) => console.warn('remoteVideoRef.play() failed:', err));
        }
        ...
        if (remoteAudioRef.current) {
          ...
          remoteAudioRef.current.play().catch((err) => console.warn('remoteAudioRef.play() failed:', err));
        }
        if (remoteVideoRef.current) {
  ...
  }, [remoteStream, callState.status, isSpeakerOn, callState.type]);
  ...
  const createPeerConnection = useCallback((stream: MediaStream) => {
    ...
    stream.getTracks().forEach((track) => {
      console.log(`WebRTC Track Lifecycle: Adding local track to PC. Kind: ${track.kind}, ID: ${track.id}, Enabled: ${track.enabled}`);
      pc.addTrack(track, stream);
    });

    // Handle incoming remote tracks
    pc.ontrack = (event) => {
      console.log(`WebRTC Track Lifecycle: Remote track received. Kind: ${event.track.kind}, ID: ${event.track.id}, Enabled: ${event.track.enabled}`);
      let rStream = remoteStream;
      if (!rStream) {
        rStream = new MediaStream();
        setRemoteStream(rStream);
      }
      if (!rStream.getTracks().includes(event.track)) {
        rStream.addTrack(event.track);
      }

      console.log(`WebRTC Track Lifecycle: Remote stream track count. Total: ${rStream.getTracks().length}, Audio: ${rStream.getAudioTracks().length}, Video: ${rStream.getVideoTracks().length}`);

      if (callState.type === 'video' && remoteVideoRef.current) {
        remoteVideoRef.current.srcObject = rStream;
        remoteVideoRef.current.muted = !isSpeakerOn;
        remoteVideoRef.current.play().catch((err) => console.warn('WebRTC Track Lifecycle: remoteVideoRef.play() failed:', err));
      } else if (callState.type === 'audio' && remoteAudioRef.current) {
        remoteAudioRef.current.srcObject = rStream;
        remoteAudioRef.current.muted = !isSpeakerOn;
        remoteAudioRef.current.play().catch((err) => console.warn('WebRTC Track Lifecycle: remoteAudioRef.play() failed:', err));
      }
    };
  ...
  const toggleSpeaker = () => {
    ...
    if (remoteVideoRef.current) {
      remoteVideoRef.current.muted = callState.type === 'video' ? !newSpeaker : true;
    }
    if (remoteAudioRef.current) {
      remoteAudioRef.current.muted = callState.type === 'audio' ? !newSpeaker : true;
    }
  };
