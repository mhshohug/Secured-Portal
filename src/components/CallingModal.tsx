...
export default function CallingModal({
  ...
  useEffect(() => {
    if (remoteStream) {
      if (remoteVideoRef.current) {
        ...
        remoteVideoRef.current.muted = true; // Keep video muted to prevent double audio/browser block
  ...
  const createPeerConnection = useCallback((stream: MediaStream) => {
    ...
    pc.ontrack = (event) => {
      ...
      if (remoteVideoRef.current) {
        ...
        remoteVideoRef.current.muted = true; // Ensure always muted to avoid double audio/browser autoplay blocks
      }
      if (remoteAudioRef.current) {
  ...
  const toggleSpeaker = () => {
    ...
    if (remoteVideoRef.current) {
      remoteVideoRef.current.muted = true; // Always keep video muted
    }
    if (remoteAudioRef.current) {
